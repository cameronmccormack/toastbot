...
const maintenanceMessage = 'Sorry, Toastbot is currently in maintenance mode. '
    + `Please try again later or, if this is unexpected, contact <@${config.ownerId}>.`;
...
export const toast = async (req: Request, res: Response): Promise<Response> => {
    ...
    if (req.body.token !== config.slackVerificationToken) return res.status(400).send();
    ...
    if (
        ...
        && !config.maintenanceBypassUserIds.some(userId => userId === toasterId)
    ) {
        return res.status(200).json({ text: maintenanceMessage });
    }
    ...
    res.status(200).json({
        text: `Ok! I'll send a toast to the <#${config.toastChannelId}> channel. Thanks for using Toastbot!`,
    });
